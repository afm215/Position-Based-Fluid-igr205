...
private:
  void buildNeighbor()
  {
      ...
      _neighbors = std::vector<std::vector<tIndex> >(_pos.size(), std::vector<tIndex>(0, 0));
      for (tIndex i = 0; i < particleCount(); i++) {
          Vec2f particle = _pos[i];
          for (int x = floor(particle.x - _kernel.supportRadius()); x <= ceil(particle.x + _kernel.supportRadius()); x++) {
              for (int y = floor(particle.y - _kernel.supportRadius()); y <= ceil(particle.y + _kernel.supportRadius()); y++) {
                  int _x = min(resX(), max(x, 0));
                  int _y = min(resY(), max(y, 0));
                  for (tIndex j = 0; j < _pidxInGrid[idx1d(_x, _y)].size(); j++) {
                      _neighbors[i].push_back(_pidxInGrid[idx1d(_x, _y)][j]);
                  }
              }
          }
  ...
  void computeDensity()
  {
      for (tIndex i = 0; i < particleCount(); i++) {
          Real d = 0;
          for (tIndex j = 0; j < _neighbors[i].size(); j++) {
              tIndex neigh = _neighbors[i][j];
              d += _m0 * _kernel.w(_pos[i] - _pos[neigh]);
          }
          _d[i] = d;
      }
      for (tIndex i = 0; i < particleCount(); i++) {
  ...
  void computePressure()
  {
      for (tIndex i = 0; i < _p.size(); i++) {
          _p[i] = max(equationOfState(_d[i], 1000, 5), 0);
      }
  }
  ...
  {   for (tIndex i = 0; i < particleCount(); i++) {
        Vec2f fpress = Vec2f(0.0, 0.0);
        for (int j = 0; j < _neighbors[i].size(); j++) {
            tIndex neigh = _neighbors[i][j];
            if (i != neigh) {
                Vec2f gradWij = _kernel.grad_w(_pos[i] - _pos[neigh]);
                fpress += (_p[i] / square(_d[i]) + _p[neigh] / square(_d[neigh])) * gradWij;
            }
        }
        _acc[i].x -= _m0 * fpress.x;
        _acc[i].y -= _m0 * fpress.y;
      }
  }
  ...
  std::vector< std::vector<tIndex> > _pidxInGrid; // data of particules in each grid
  std::vector<std::vector<tIndex> > _neighbors;  // neighbors of particle i 
